...
function menuFetch(req, res, next) {
  ...
  menu.find({ quantity: { $gt: 0 }, donated: false, geojson: { $near: { $geometry: { type: "Point", coordinates: req.temp_user.geojson.coordinates }, $maxDistance: (1000 * 1000) } } }, {}, { /*sort: '-created_at'*/ })
    .limit(req.query.limit * 1)
    .skip((req.query.page - 1) * req.query.limit)
    .populate('owner', ['_id', 'name'])
    .lean()
    .then(async (data) => {
      let total_documents = await menu.countDocuments({ quantity: { $gt: 0 }, geojson: { $geoWithin: { $centerSphere: [req.temp_user.geojson.coordinates, ((1000) / 1.60934) / 3963.2] } } })
...
function menuRecommendedFetch(req, res, next) {
  ...
  menu.find({ quantity: { $gt: 0 }, donated: false, food_type: req.temp_user.fav_food_type, geojson: { $near: { $geometry: { type: "Point", coordinates: req.temp_user.geojson.coordinates }, $maxDistance: (1000 * 1000) } } }, {}, { /*sort: '-created_at'*/ })
    .limit(req.query.limit * 1)
    .skip((req.query.page - 1) * req.query.limit)
    .populate('owner', ['_id', 'name'])
    .lean()
    .then(async (data) => {
      let total_documents = await menu.countDocuments({ quantity: { $gt: 0 }, food_type: req.temp_user.fav_food_type, geojson: { $geoWithin: { $centerSphere: [req.temp_user.geojson.coordinates, ((1000) / 1.60934) / 3963.2] } } })
      let total_pages = Math.ceil(total_documents / req.query.limit)
      req.total = total_documents
      req.total_pages = total_pages
      req.temp_menu = data
